<template>
	<view>
		...
		<view style="position: fixed;top: 0;width: 100%;z-index: 100;">
			<headers title="说说详情" :leftIcon="true" rightText="删除" @rightClick="rightClick"/>
		</view>
			...
			<view class="box">
				...
				<view class="content">
					<view class="msg">
						...
						<view class="img_list">
							<image v-for="(item,index) in obj.imgList" @click="seeImg(obj.imgList)" :src="item" mode="widthFix"></image>
						</view>
					</view>
					<view class="bottom">
				...
				<view class="content">
					<view class="msg">
					    ...
					    <view class="video" @click="seeVideo">
					    	<image class="poster" :src="obj.controimg" mode="widthFix"></image>
		...
		<uni-popup type="center" ref="video" @change='change' :maskClick="false">
			<view class="video_popup">
				<view class="video_close">
					<image @click="close" src="../../../static/me/guanbi.png" mode="widthFix"></image>
				</view>
				<video v-if="flog" object-fit="cover"  :src="obj.circle_video" controls></video>
			</view>
		</uni-popup>
	</view>
</template>
		...
		methods:{
			rightClick(){
				...
				if(this.status==='image'){
					url='/api/Groundcircle/DelGroundcircle'
				}else if(this.status==='video'){
					url='/api/Groundcircle/DelCircleVideo'
				}
			...
			seeVideo(){
				uni.navigateTo({
					url:`./videoPage?url=${this.obj.circle_video}`
				})
