<template>
	<view>
		...
		<view style="position: fixed;top: 0;width: 100%;">
			<headers title="比赛订单详情" :leftIcon="true" :rightText="rightText" @rightClick="rightClick"/>
		</view>
		...
		<view class="detail">
			...
			<view class="time">
				<text v-if="Obj.activity_status==='1'"><text class="title">活动状态</text>:待发布</text>
				<text v-if="Obj.activity_status==='2'"><text class="title">活动状态</text>:已发布</text>
				<text v-if="Obj.activity_status==='3'"><text class="title">活动状态</text>:比赛中</text>
				<text v-if="Obj.activity_status==='4'"><text class="title">活动状态</text>:结束比赛</text>
				<text v-if="Obj.activity_status==='5'"><text class="title">活动状态</text>:赛事完成</text>
				<text v-if="Obj.activity_status==='6'"><text class="title">活动状态</text>:异常</text>
			</view>
			<view class="time"><text><text class="title">订单号</text>:{{Obj.ordersn}}</text></view>
			...
			<view class="time"><text><text class="title">比赛开始时间</text>:{{Obj.pt_starttime}}</text></view>
			<view class="time"><text><text class="title">比赛结束时间</text>:{{Obj.pt_stoptime}}</text></view>
			<view class="time">
				<text><text class="title">时间</text>:{{Obj.ordercrea_time}}</text>
				<view class="">
			...
			<view class="time" v-if="Obj.paytotal_fee!=='0'"><text><text class="title">交易流水号</text>:{{Obj.paytransaction}}</text></view>
			<view class="time"  v-if="Obj.paytotal_fee!=='0'"><text><text class="title">实际支付</text>:{{Obj.paytotal_fee}}</text></view>
			<view class="time"  v-if="Obj.paytotal_fee!=='0'"><text><text class="title">支付时间</text>:{{Obj.paytime}}</text></view>
			<view class="content"><text><text class="title">活动内容</text>:{{Obj.activity_intro}}</text></view>
			<view class="fn">
				<view class="pay" @click="pay" v-if="Obj.status==='1'"><text>支付</text></view>
				<view class="pay" @click="cancelPay" v-if="Obj.status==='1'"><text>取消订单</text></view>
				<view class="pay" @click="deleteOrder" v-if="Obj.status==='0'||Obj.status==='1'||Obj.activity_status==='5'||Obj.status==='3'"><text>删除订单</text></view>
				<view class="pay" @click="seeGame" ><text>查看活动</text></view>
			</view>
			<!-- 
			<view class="fn">
				...
				<text @click="updateRank">上传成绩</text>
				</view>
				<view class=""
					 v-if="(Obj.status==='1'||Obj.status==='2')&&userinfo.user_id===Obj.userid"
		...
		<view class="people">
			...
			<view class="item" v-for="(item,index) in Obj.userlist" :key="index">
				<view class="user">
					...
					<text v-if="userinfo.username===Obj.username">{{item.username}}</text>
				</view>
				<view style="font-size: 26rpx;"  v-if="userinfo.username===Obj.username"><text>{{item.userphone}}</text></view>
				<!-- <view class=""  
	...
	export default {
		...
		methods:{
			async deleteOrder(){
				...
				let res = await this.$Post('/api/Groundactivity/UserFishGroupActivityDel',obj);
			...
			async cancelPay(){
				///api/Groundactivity/UserFishGroupActivityClose
				let obj ={token:this.token,id:this.Obj.id};
				let res = await this.$Post('/api/Groundactivity/UserFishGroupActivityClose',obj);
				this.$Toast(res.data.msg)
				if(res.data.code===1){
					setTimeout(()=>{
						uni.navigateBack({
			...
			seeGame(){
				uni.navigateTo({
					url:`../../club/pinchFish/pinchFishDetail?id=${this.Obj.activityid}`
				})
			},//查看订单的活动详情
			payConfirm(e){
				console.log(e);
			...
			pay(){
				let time1 = new Date(this.Obj.signup_stoptime).getTime();
				let time2 = new Date().getTime();
				if(time1<time2){
					this.$Toast('已过报名时间');
					return
				}
				if(this.Obj.actuser_num===this.Obj.pt_number){
					this.$Toast('人数已满，不能参与');
					return
				}
			...
			async getMsg(activityid){
				// /api/Groundactivity/UserFishGroupActivityInfo
				///api/Groundactivity/FishGroupActivityinfo
				let res = await this.$Post('/api/Groundactivity/UserFishGroupActivityInfo',{id:activityid,token:this.token});
				if(res.data.code===1){
